...
# --- 1. 頁面配置 ---
st.set_page_config(page_title="台股 AI 戰情室 2.4", layout="wide", page_icon="🏢")

# CSS 補丁：確保手機端與電腦端表格整齊
st.markdown("""
    <style>
    .js-plotly-plot .plotly .main-svg { touch-action: pan-y pinch-zoom !important; }
    div[data-testid="stTable"] { font-size: 1.1rem; }
    </style>
...
def get_stock_info(sid):
    ...
    for suffix in [".TW", ".TWO"]:
        try:
            ...
            # 優先抓取 shortName (通常包含中文)
            name = t.info.get('shortName') or t.info.get('longName') or sid
            return name, f"{sid}{suffix}"
        except:
            continue
    return sid, None

@st.cache_data(ttl=600)
def get_stock_history(target_id):
    """專門抓取歷史股價"""
    try:
        df = yf.Ticker(target_id).history(period="1y")
...
if not stocks:
    ...
    summary_data = []
    with st.spinner('📢 正在同步全球數據與中文名稱...'):
        for s in stocks:
            name, tid = get_stock_info(s)
            if tid:
                df = get_stock_history(tid)
                if not df.empty:
                    cp = df['Close'].iloc[-1]
                    ...
                    summary_data.append({"股名": name, "代號": s, "股價": f"{cp:.2f}", "狀態": status})
    
    if summary_data:
        st.table(pd.DataFrame(summary_data))
    else:
        st.warning("⚠️ 暫時無法獲取數據，請稍後重試或檢查網路。")
    ...
    if focus_target:
        name, tid = get_stock_info(focus_target)
        if tid:
            df = get_stock_history(tid)
            if not df.empty:
                # 計算指標
